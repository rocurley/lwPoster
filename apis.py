...
def load_boilerplate(config):
    ...
    if "phone" in location_config:
        phone = "%s (general meetup info at %s)"%(location_config["phone"], phone)
...
def lw2_post_meetup(topic, config, public):
    ...
    return lw2_post_meetup_raw(lw_key,
                       maps_key,
                       title,
                       body,
                       location["str"],
...
def lw2_post_meetup_raw(lw_key, maps_key, title, body, location, date, startTime, endTime, groupId, public):
    ...
    geocoding_resp = requests.get("https://maps.googleapis.com/maps/api/geocode/json",
            params={
                "address": location,
                "key": maps_key,
            })
    ...
    resp = requests.post("https://www.lesswrong.com/graphql",
                    json=request,
...
""")
    resp = requests.post("https://www.lesserwrong.com/graphql",
...
def fb_post(fb_cookies, fb_dstg, title, description, location, date, time, public=False):
...
def fb_post_meetup(topic, config, public=False):
    ...
    res = fb_post(fb_cookies, fb_dstg,
        title = title,
        description = description,
...
def ssc_meetup_text(title, location, date, time, header, footer, email, phone, lw_url=None):
    if lw_url:
...
{footer}""".format(time=time.strftime("%I:%M %p"),
...
def update_ssc_meetup(title, config, public, lw_url=None):
    ...
    with open("ssc-meetups/index.md","r+") as f:
        ...
        header = "\n".join(lines[:2])
        footer = "\n".join(lines[-2:])
        new = r.sub(ssc_meetup_text(title, location, date, time, header, footer, email, phone, lw_url),old)
        f.seek(0)
        f.write(new)
        f.truncate()
    print_command(["git", "add", "index.md"], cwd = "ssc-meetups")
    print_command(["git", "commit", "-m",
                   date.strftime("San Francisco Meetup %F")], cwd = "ssc-meetups")
    if public:
